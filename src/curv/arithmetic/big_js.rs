/*
    ...
    browser-native JS BigInt via wasm-bindgen when compiled for wasm32.
    On native targets, falls back to num-bigint's implementations.

    Activate with: --no-default-features --features js-bigint-backend
*/
...
")]
extern "C" {
    fn js_mod_pow(base_hex: &str, exp_hex: &str, mod_hex: &str) -> String;
    fn js_mod_mul(a_hex: &str, b_hex: &str, mod_hex: &str) -> String;
    fn js_mod_inv(a_hex: &str, mod_hex: &str) -> String;
}
...
// ============== Modulo: JS BigInt FFI on wasm32, num-bigint fallback on native ==============

impl Modulo for BigInt {
    fn mod_pow(base: &Self, exponent: &Self, modulus: &Self) -> Self {
        #[cfg(target_arch = "wasm32")]
        {
            let result_hex = js_mod_pow(
                &base.to_hex(),
                &exponent.to_hex(),
                &modulus.to_hex(),
            );
