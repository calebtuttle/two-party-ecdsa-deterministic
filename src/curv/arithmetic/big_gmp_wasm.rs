/*
    GMP-WASM-accelerated BigInt backend.

    Identical to big_num.rs (uses num-bigint internally), but delegates
    the hot-path Modulo operations (mod_pow, mod_mul, mod_inv) to
    gmp-wasm (GMP compiled to WASM) via wasm-bindgen when compiled for wasm32.
    On native targets, falls back to num-bigint's implementations.

    The gmp-wasm library must be initialized before any WASM calls:
      const { init } = require('gmp-wasm');
      const gmp = await init();
      globalThis.__gmpWasm = gmp;

    Activate with: --no-default-features --features gmp-wasm-backend
*/
...
// ============== JS FFI (wasm32 only) — calls gmp-wasm via globalThis.__gmpWasm ==============
...
")]
extern "C" {
    fn gmp_mod_pow(base_hex: &str, exp_hex: &str, mod_hex: &str) -> String;
    fn gmp_mod_mul(a_hex: &str, b_hex: &str, mod_hex: &str) -> String;
    fn gmp_mod_inv(a_hex: &str, mod_hex: &str) -> String;
}
...
// ============== Modulo: gmp-wasm FFI on wasm32, num-bigint fallback on native ==============

impl Modulo for BigInt {
    fn mod_pow(base: &Self, exponent: &Self, modulus: &Self) -> Self {
        #[cfg(target_arch = "wasm32")]
        {
            let result_hex = gmp_mod_pow(
                &base.to_hex(),
                &exponent.to_hex(),
                &modulus.to_hex(),
            );
            BigInt::from_hex(&result_hex)
        }
...
mod tests {
    ...
    #[test]
    fn bench_mod_pow_2048() {
        let base = BigInt::sample(2048);
        let exp = BigInt::sample(2048);
        let modulus = BigInt::sample(2048);
        let start = std::time::Instant::now();
        for _ in 0..10 {
            let _ = BigInt::mod_pow(&base, &exp, &modulus);
        }
        let elapsed = start.elapsed();
        println!("10x mod_pow(2048-bit): {:?} ({:?} per op)", elapsed, elapsed / 10);
    }
}
